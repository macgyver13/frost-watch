#!/usr/bin/env python3
"""Tests for GitHub search-backed live collection."""
...
class GitHubLiveCollectorTests(unittest.TestCase):
    def test_github_repository_search_requires_relevant_content_not_only_name_match(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                items, projects, sources = build_seed_feed.build_items(cfg, github_repo_fetcher=fake_fetch)

                self.assertEqual(len(items), 1)
                self.assertEqual(items[0]["source_url"], "https://github.com/deegalabs/konclave")
    ...
    def test_existing_github_discovery_keeps_original_discovered_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                items, _projects, sources = build_seed_feed.build_items(cfg, github_repo_fetcher=fake_fetch)
                self.assertEqual(items[0]["discovered_at"], "2026-08-25T00:00:00Z")
                self.assertEqual(items[0]["event_time"], "2026-08-25T00:00:00Z")
                self.assertEqual(sources["gh-search:repo-discovery:example-frost-dkg"]["discovered_at"], "2026-08-25T00:00:00Z")
            finally:
                build_seed_feed.OUT = old_out
